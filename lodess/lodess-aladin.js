...
var lodess_aladin = function(aladin_div, requestedOptions) {

    Aladin.DEFAULT_OPTIONS.target = "156 +60";
    Aladin.DEFAULT_OPTIONS.fov = 150;
    Aladin.DEFAULT_OPTIONS.survey = "LoDeSStest";
    Aladin.DEFAULT_OPTIONS.lodess_pointings_url = "https://home.strw.leidenuniv.nl/~wwilliams/lodess/pointings_lodess.json";
    Aladin.DEFAULT_OPTIONS.lodess_pointings_radius = 4.37;
    HpxImageSurvey.SURVEYS.push(
	{
           "id": 'LoDeSStest',
           "url": "https://home.strw.leidenuniv.nl/~wwilliams/lodess/hips_lodess/",
           "name": 'LoDeSS-test',
           "maxOrder": 7,
           "frame": 'equatorial',
           "format":'png'
    ...
    var popup = function(name, pstatus, obs) {
        ...
        if (obs.length == 0) {
            ...
            for (var i = 0; i < obs.length; i++) {
                ...
                desc = desc + "<br>&nbsp;&nbsp;Obs Length: " + obs[i].int;
    ...
    var add_pointing = function(pointing) {
        ...
        var obs = pointing[4];
        var radius = aladin.options.lodess_pointings_radius;
        if (obs.length > 0) {
    ...
    var add_survey_pointings = function() {
        ...
        if (json_data === undefined) {
            $.getJSON(aladin.options.lodess_pointings_url).done(
                function(response) {
                    json_data = response;
